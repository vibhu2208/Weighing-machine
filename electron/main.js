...
const fs = require('fs');
const { app, BrowserWindow, dialog, Menu, shell, protocol, net } = require('electron');
...
function registerLocalMediaProtocol() {
  const { PATHS, normalizePath } = require('../backend/utils/fileStorage');

  protocol.handle('weighbridge-local', async (request) => {
    try {
      let encoded = request.url.slice('weighbridge-local://'.length);
      ...
      const uploadsRoot = normalizePath(PATHS.UPLOADS);
