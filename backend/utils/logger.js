...
class RotatingJsonFile {
  constructor(filename, baseName) {
    this.filePath = getLogPath(filename);
  ...
  write(info) {
    try {
      rotateLogFile(this.filePath, this.baseName);
      if (!fs.existsSync(this.filePath)) fs.writeFileSync(this.filePath, '', 'utf8');
      fs.appendFileSync(this.filePath, `${JSON.stringify(buildEntry(info))}\n`, 'utf8');
    } catch (err) {
...
class FileRouterTransport extends Transport {
  log(info, callback) {
    ...
    if (info.meta && info.meta.type === 'device') deviceSink.write(info);
    callback();
  }
}
